...
bb = {
    ...
    style: function(root) {
        ...
        if (bb.textInput)
            bb.textInput.apply(root.querySelectorAll('input[type=text], [type=password], [type=tel], [type=url], [type=email], [type=number], [type=date], [type=time], [type=datetime], [type=month], [type=datetime-local], [type=color], [type=search]'));
        if (bb.dropdown)
    ...
    removePreviousScreenFromDom: function() {
        var numItems = bb.screens.length,
                ...
                stepBack;
        if (numItems == 1)
            return; // There is only one screen on the stack
...
bb.actionBar = {
    apply: function(actionBar, screen) {
        ...
        actionBar.setSelectedTab = function(tab) {
            if (tab.getAttribute('data-bb-style') != 'tab')
                return;
...
_bb_progress = {
    ...
    style: function(progress, offdom) {
        ...
        progress.setMax = function(value) {
            if (!value || (value < 0) || (value == this.max))
                return;
            this.max = value;
...
_bb10_button = {
    ...
    style: function(outerElement) {
        ...
        if (outerElement.hasAttribute('data-mrk-alert')) {
            normal = normal + " alertbutton";
        }
        if (bb.device.newerThan10dot1) {
    ...
    apply: function(elements) {
        ...
        for (i = 0; i < elements.length; i++) {
            ...
            outerElement.refresh = function(listItems) {
                if (!listItems || !listItems.length || (listItems.length <= 0))
                    return;
                var i,
                        item,
